...
(function ($) {
    $.extend({
        ...
        modal: {
            ...
            // 弹出层指定宽度
            openIndex: function (title, url, width, height, callback) {
                //如果是移动端，就使用自适应大小弹窗
                if (navigator.userAgent.match(/(iPhone|iPod|Android|ios)/i)) {
                    width = 'auto';
                    height = 'auto';
        ...
        operate: {
            ...
            detailTab: function (id) {
                $.modal.openTab("明细" + $.table._option.modalName, $.operate.detailUrl(id));
            },
            ...
            verify: function (url, data, callback) {
                var config = {
                    url: url,
                    type: "post",
                    dataType: "json",
                    ...
                    success: function (result) {
                        ...
                        $.operate.successCallbackForVerify(result);
                    }
                };
                $.ajax(config)
            },
            ...
            successCallbackForVerify: function (result) {
                test2('退回成功');
                $.modal.closeTab()
            },
            // 选项卡成功回调执行事件（父窗体静默更新）
...
function forbidBackSpace(e) {
    ...
    var c = $(obj).attr('contenteditable');
    if (ev.keyCode == 8 && t == undefined && c != "true") return false;
}
/** 表格类型 */
table_type = {
    bootstrapTable: 0,
